...
Investigate locally with `rg`, targeted file reads, test inspection, and `git log` on relevant paths. Your goal is to find:
...
Only use `spawn_agent` if the user explicitly asks for delegation or parallel agent work.
...
Create a GitHub issue with the tooling available in the environment when the user asked for issue creation. Prefer connector lookups for repository context and use `gh issue create` when needed. If issue creation is unavailable, write the issue locally with the template below and tell the user where it is.
